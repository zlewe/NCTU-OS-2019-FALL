...
struct sembuf p = { 0, -1}; //semwait
struct sembuf v = { 0, +1}; //semsignal
...
void downloader(int target_id){
	...
	//enter critical section
	    ...
	    //exit critical section (eof)
            /* TODO: signal writer */
            if(sem_signal(sem_id) == -1){
	    	perror("signal error\n");
		exit(1);
	    }
	    if(sem_signal(empty_id) == -1){
                perror("signal error\n");
                exit(1);
    	    }
...
void IPC_init(){
    ...
    printf("mutex_id: %d\n", sem_id);
    if((empty_id = semget(IPC_PRIVATE, 1, IPC_CREAT | 0666)) < 0){
        perror("semget error\n");
        exit(1);
    }
    printf("empty_id: %d\n", empty_id);
    if((full_id = semget(IPC_PRIVATE, 1, IPC_CREAT | 0666)) < 0){
        perror("semget error\n");
        exit(1);
    }
    printf("full_id: %d\n", full_id);
    ...
    sem_union.val = BUFNUM;
    if(semctl(full_id, 0, SETVAL, sem_union) == -1){
        perror("semctl error\n");
        exit(1);
    }
    sem_union.val = 0;
    if(semctl(empty_id, 0, SETVAL, sem_union) == -1){
        perror("semctl error\n");
        exit(1);
    }
...
void IPC_release(){
    ...
    union semun sem_union;
    printf("release semaphores\n");
    if (semctl(sem_id, 0, IPC_RMID, sem_union) == -1){
	perror("release error\n");
	exit(1);
    }
    if (semctl(empty_id, 0, IPC_RMID, sem_union) == -1){
        perror("release error\n");
        exit(1);
    }
    if (semctl(full_id, 0, IPC_RMID, sem_union) == -1){
        perror("release error\n");
        exit(1);
    }
